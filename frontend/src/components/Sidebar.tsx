...
import { LayoutDashboard, MapPin, Compass, LogOut, X, User } from "lucide-react";
...
export default function Sidebar({ activeTab, isMobileOpen, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-white border-r border-indigo-100/50 flex flex-col p-6 shadow-2xl transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:shadow-none shrink-0 ${
          ...
          <nav className="space-y-2">
            ...
            >
              <div className="flex items-center gap-3">
                <LayoutDashboard size={18} className={iconClass(activeTab === "dashboard")} />
                Dashboard
              </div>
            </button>
            <button
            ...
            >
              <div className="flex items-center gap-3">
                <MapPin size={18} className={iconClass(activeTab === "saved")} />
                Saved Places
              </div>
            </button>
            <button
            ...
            >
              <div className="flex items-center gap-3">
                <User size={18} className={iconClass(activeTab === "profile")} />
                Profile
              </div>
            </button>
          </nav>
        ...
        <div className="mt-6 pt-6 border-t border-indigo-100/50 shrink-0">
          <div className="flex items-center gap-3 px-2 mb-6 bg-indigo-50/50 p-3 rounded-2xl border border-indigo-100/50">
            ...
            <div className="overflow-hidden">
              <p style={{ fontFamily: '"Sora", sans-serif' }} className="text-sm font-bold text-[#0d1f5c] truncate">
